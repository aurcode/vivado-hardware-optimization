...
typedef struct {
    ...
    XMlp_accel_8b_uio_map maps[ MAX_UIO_MAPS ];
} XMlp_accel_8b_uio_info;

/***************** Variable Definitions **************************************/
static XMlp_accel_8b_uio_info uio_info;
...
static int uio_info_read_name(XMlp_accel_8b_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMlp_accel_8b_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMlp_accel_8b_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMlp_accel_8b_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMlp_accel_8b_Initialize(XMlp_accel_8b *InstancePtr, const char* InstanceName) {
	XMlp_accel_8b_uio_info *InfoPtr = &uio_info;

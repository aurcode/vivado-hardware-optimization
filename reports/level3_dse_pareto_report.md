...
2. **$8 \le W < 11$ Benign Degradation Zone**: At $W = 8$ ($F = 5$), the noise ratio is $15.8\times$ smaller than the 10th percentile margin. The SNR remains high ($28.4 \text{ dB}$), preserving 98.00% accuracy.
3. **$W \le 4$ Catastrophic Collapse Zone**: At $W = 4$ ($F = 2, \Delta = 0.25$), quantization step noise balloons. The differential noise $\sqrt{2}\sigma_{z2} \approx 1.50$ reaches the same order of magnitude as the class decision margin. For samples with margins below $2.96$, the flip probability surges to over $2.4\% \text{ to } 50\%$. The signal-to-noise ratio collapses to **$8.0 \text{ dB}$**, causing widespread decision flips and degrading recognition accuracy from $98.00\%$ to **$85.00\%$** ($-13.00\%$ drop).
...
$$\forall i, \ \text{Metric}_i(A) \ge \text{Metric}_i(B) \quad \text{and} \quad \exists j, \ \text{Metric}_j(A) > \text{Metric}_j(B)$$
...
1. **16-bit High-Precision is STRICTLY DOMINATED**:
   - Comparing 16-bit ($W=16$) vs 11-bit Baseline ($W=11$):
     ...
     - $\text{LUT}(11\text{b}) = 2,100 < \text{LUT}(16\text{b}) = 3,200$ ($-34.4\%$ savings)
     - $\text{FF}(11\text{b}) = 2,280 < \text{FF}(16\text{b}) = 3,450$ ($-33.9\%$ savings)
     - $\text{BRAM}(11\text{b}) = 2 < \text{BRAM}(16\text{b}) = 4$ ($-50.0\%$ savings)
     - $\text{Latency}(11\text{b}) = 31.8 \ \mu\text{s} < \text{Latency}(16\text{b}) = 32.4 \ \mu\text{s}$
     - $\text{Timing Slack}(11\text{b}) = +2.45 \text{ ns} > \text{Slack}(16\text{b}) = +1.82 \text{ ns}$
...
2. **Identification of the Non-Dominated Pareto Frontier**:
   ...
   - **Frontier Point 3: 8-bit Parallel-32 (Throughput-Optimal)**:
     ...
     *Doubles SIMD width to 32 lanes, cutting inference latency by $50\%$ to $16.1 \ \mu\text{s}$ and elevating throughput to $62,111 \text{ FPS}$.*

...
If $\bar{Y}_{\text{corner}} > 127.0$, the image is classified as white paper and inverted:
...
$$Y_{\text{clean}}(x, y) = \begin{cases} Y_{\text{inv}}(x, y), & \text{if } Y_{\text{inv}}(x, y) \ge 0.7 \cdot T_{\text{otsu}} \\ 0, & \text{if } Y_{\text{inv}}(x, y) < 0.7 \cdot T_{\text{otsu}} \end{cases}$$
...
The spatial bounding box $[y_{\min}, y_{\max}, x_{\min}, x_{\max}]$ enclosing all active foreground pixels ($Y_{\text{clean}}(x, y) > 0.5 \cdot T_{\text{otsu}}$) is extracted. This strips uninformative margins and focuses computation purely on the digit stroke.
...
Each neuron $j$ in FC1 learns a rigid, position-sensitive template over the $28 \times 28$ image grid. Although Center-of-Mass alignment normalizes the global translation of the stroke cluster, it cannot correct for **local structural deformations** (e.g., stroke slant, loop aspect ratio, or vertical ascender curvature). If a handwriting stroke is tilted by even $2\text{--}3$ pixels, it shifts out of its trained excitatory receptive field and into inhibitory synaptic zones.

### 6.2 Stroke Width Discrepancy (Ballpoint Pen vs. MNIST Antialiasing)
- **MNIST Baseline**: Digit strokes in standard MNIST were generated by normalized rasterization followed by Gaussian-like anti-aliasing filter smoothing, producing strokes with an average width of **$3.0 \pm 0.5$ pixels** and soft edge gradients.
- **Real-World Handwriting**: Ballpoint pens and mechanical pencils on paper create sharp, high-contrast, binary-like strokes with a physical width of only **$1.0 \text{ to } 1.8$ pixels**.
...
- When inverted, faint shadow boundaries bleed into the active stroke mask, expanding the bounding box by $4\text{--}8$ pixels and artificially shrinking the scaled digit inside the $20 \times 20$ box.
...
- **The Bias-Free Vulnerability**: In standard neural networks with learnable biases ($z = Wx + b$), negative bias terms ($b_j < -0.5$) act as a natural noise barrier, requiring a minimum stroke activation before ReLU activates. In our hardware-optimized **bias-free network ($b \equiv 0$)**, any stray positive pixel value $X_i > 0$ paired with a positive weight $W_{1, ji} > 0$ directly produces a non-zero activation.
- The 0.7x Otsu background cutoff successfully eliminated $98\%$ of paper grain; however, residual speckles near the stroke boundary subtly pull the Center-of-Mass centroid by $0.8 \text{ to } 1.4$ pixels, triggering spatial misalignment.
...
1. **Tier 1: Preprocessor Energy Gate**:
   ...
   - **Rejection Condition**: If $I_{\max} < 0.20$ OR $E_{\text{stroke}} < 8.0$, the image is classified as background noise and dropped before invoking the hardware accelerator.
...
2. **Tier 2: Hardware Peak Logit Confidence Gate**:
   ...
   - **Rejection Condition**: If peak logit $z_{\max} = \max_c(z_{2, c}) < \tau_{\text{reject}} = 1.0$, the inference is flagged as an Out-of-Distribution (OOD) event.
   - *Empirical Basis*: As demonstrated in Section 3.2, genuine digits produce an average peak logit of **$4.35 \text{ to } 7.60$** (minimum $2.15$). Blank controls produce an average peak logit of only **$0.10$** (maximum $0.15$).
...
1. **Processing System (ARM Cortex-A9 Host)**:
   ...
   - Profiling shows full preprocessing requires **$1.85 \text{ ms}$ per frame** on Cortex-A9 @ 667 MHz.
...
2. **Programmable Logic (FPGA Accelerator Core)**:
   - Executes `mlp_accel` inference in **$31.8 \ \mu\text{s}$**.
...
3. **End-to-End System Frame Rate**:
   ...
   Achieves **$\approx 526 \text{ frames per second}$**, vastly exceeding standard 30/60 FPS camera streaming requirements.
